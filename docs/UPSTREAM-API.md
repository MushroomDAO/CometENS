...
> `/register` 在某些部署下有额外限制(见[错误码](#错误码)表)。`/v1/register` 不受影响。
...
本文档描述的是**本仓库代码**的行为。参考部署(`cometens-api.jhfnetboy.workers.dev`)
上**还没有**申请/审批那组端点 —— `/apply` `/approval-mode` `/applications` `/approve`
目前返回 404(2026-09-04 实测)。`/register` 与 `/v1/register` 正常。

所以 `APPROVAL_REQUIRED` 这个错误码在那台 worker 上暂时不会出现:
`APPROVAL_MODE` 只有在部署了这组端点的实例上才有意义。

集成前先跑 `pnpm check:deploy-order`(可用 `--api-url` 指向你自己的实例)——
**它读的是活的部署,不是这份文档。**
