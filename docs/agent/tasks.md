...
> **动手之前先读 [`practices.md`](practices.md)。** 那里的规程都来自这个仓库真实犯过的错:
>
> | 规程 | 一句话 |
> |---|---|
> | 合并前协议 | 「可干净合并」只说明文本不打架,不说明合并后的仓库仍然成立 |
> | 取证规程 | 判据的作用域必须和问题的作用域一样大 |
> | 理由要经得起核 | 结论对、理由错的注释比没有注释更危险 |
> | 量取规程 | 同一个量,读法不同会得出不同的数 |
...
>
> 1. practices.md 里给某条加一条新要求时,这一行**不需要跟着改**(需要改 = 它是副本)。
> 2. practices.md 的每个 `###` 小节,这张表里都有且只有一行,**名字逐字相同**。
>
> 只有第一条时,**新增一整条规程不属于「给某条加新要求」,判据根本不会触发** ——
> 那不是被违反,是它管不到。第二条由 `test/unit/practices-index.test.ts` 机械守住。
>
> 名字和一句话留在这里,是因为 `pilot run` 按本文件选 task ——
> **写在别处的更严格版本到不了执行者手上**(同 §T1.3.1 的那条注)。完整版与实例在 practices.md。
...
### T1.7.2 把 typecheck 覆盖到 gateway worker  `DONE (PR #74)`
...
- **2026-09-04 现状**:M1 验收那一半**只差一条,而它不是工程量**:

  | 未决 | 谁能解除 |
  |---|---|
  | A3 陌生开发者 2 小时跑通 | **一个没参与过这个仓库的人** |
  | ~~界面验收第 5 条~~ | ✅ 已解除 2026-09-04 —— 维护者看过线上落地页后判定通过 |
  | ~~B2 撤销可验证~~ | ✅ 已验 2026-09-04 —— 那把 L1 私钥一直在 `.env.local` 里 |
  | ~~线上缺 4 个端点~~ | ✅ 已解除 2026-09-04 —— worker 已部署,`check:deploy-order` 报 all 16 present |

  **只剩 A3,而它要的是一个外部的人**(我按定义不在那个总体里)。
  再跑多少次判据都不会解除 —— **这不是「还没做」,是「做不了」**,
  记在这里免得下一个人把它当成待办去追。

  (B2 曾与它并列,理由是「要一把私钥」;2026-09-04 发现那把钥匙一直在 `.env.local` 里,
  当天验掉。**「缺条件」和「没人去试」在台账上长得一模一样。**)

  > **这张表下面只留一份散文,是刻意的。** 上一版这里挂着三份副本 ——
  > 表下第 2 行一份、第 8 行一份(内容相同),外加一句散文说「第四条已解除」
  > 而表里只有三行。**A3 解除那天,人眼能够到第 2 行,够不到第 8 行。**
  > 那第四条也已经收进表里,所以现在表是唯一的源。
  > (PR#106 评审:「副本能有清单,只要让它只剩一份」。)
