...
- **2026-09-04 现状**:M1 验收那一半**已经不远了,但还差三条,而它们不是工程量**:
  ...
  **只剩 A3。** 它要一个**外部的人**(我按定义不在那个总体里)—— 不是工程量,
  所以再跑多少次判据都不会解除。
  (B2 曾与它并列,理由是「要一把私钥」;2026-09-04 发现那把钥匙一直在 `.env.local` 里,
  当天验掉。**「缺条件」和「没人去试」在台账上长得一模一样。**)

  第四条(线上缺 4 个端点)已于 2026-09-04 解除 —— worker 已部署,
  `check:deploy-order` 报 all 16 endpoints present,详见 `progress.md`〈部署与验收〉。

  **注意前两条需要的是同一样东西:一个外部的人。** 我按定义不在那两个总体里,
