...
2. ~~**界面验收第 5 条 —— 落地页能让一个不懂链的人在 30 秒内说出这是干嘛的。**~~
   ✅ **已解除(2026-09-04,维护者判定通过)。** 判据要的读数是「一个外部的人的实际反应」,
   而维护者本人就在那个总体里,他看过线上落地页后判定通过。
   **这条从一开始就只能这样解除** —— 不是靠再跑一次判据。原文保留:
...
3. ~~**B2 —— 撤销可验证**~~ ✅ **已验 2026-09-04**(`pnpm verify:b2`,tx `0xf0d556f9…` / `0x2bf77cd4…`)。
   此前记作「需要域名持有者的 L1 私钥」—— **那把钥匙一直在 `.env.local` 里**,`0xb5600060…` 就是 `aastar.eth` 的 owner。
...
| 2. 界面验收第 5 条(不懂链的人看落地页)| ✅ **已解除**(2026-09-04 维护者判定) |
| 3. B2 撤销可验证 | ✅ 已验 2026-09-04 —— 钥匙一直在手上,只是没人把它和判据对上 |
| 4. 线上缺 4 个端点 | ✅ **已解除** |

**只剩 A3 一条,而它要的是一个外部的人。** 它不是被工程量卡住的,
是被「我按定义不在那个总体里」卡住的 —— 再跑多少次判据都不会解除。

（这句原本写「剩下三条里有两条」。B2 解除时表格改了、这句没跟上 ——
就是本文件〈台账会漂,而且是同一个成因〉说的那个成因,第 N 次。
**散文里的计数是表格的副本,而副本不会跟着源一起变。**）
...
| # | 决定 | 后果 |
|---|---|---|
| 1 | **Resolver 形态保持不动** | FU-11 那 4 格集成测试**长期红**,原因已知且记全。谁将来要动都有读数可查,不必重查一遍链 |
| 2 | Alchemy 那个 app 不去开 Optimism 网络 | OP 走公共端点(`sepolia.optimism.io`),线上服务已验证可用。**观测**:同一个 key 打 `eth-sepolia`/`eth-mainnet` 返回 chainId,打 `opt-sepolia`/`opt-mainnet` 返回 `is not enabled for this app` |
| 3 | **界面验收第 5 条:通过** | M1 未决从四条降到三条 |
| 4 | 其余页面**不做**三语 | 落地页默认英文,`/lookup` `/register` `/admin` 仍是中文 —— **一个英文访客点第一次就会掉出英文**,这是已知且接受的形态 |

> **第 2 条按观测记,不按推断的原因记。** 我原本写的是「key 没问题,是 app 的网络开关没勾」——
> 那是**推断**。同样符合那条报错的还有「该计划/套餐不含 OP 网络」,
> 而**两者对应的动作不同**(点一下开关 vs 换套餐)。评审指出的。
> 观测本身足够支撑那个决定(走公共端点),不需要那一步推断。

第 4 条的后果写在这里,是因为它**看起来像个遗漏而实际是个决定**。下一个人看到英文落地页配中文子页,
第一反应会是「没做完」;写下来才分得开「没做」和「不做」。
...
2. ~~**B2 —— 撤销可验证**~~ ✅ **已验 2026-09-04**,见 acceptance.md 第 2 条。
...
| B2 撤销可验证 | ✅ 已验 2026-09-04(`pnpm verify:b2`) |
