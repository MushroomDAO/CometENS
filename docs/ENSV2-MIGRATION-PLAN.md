...
> ## 🔴 §3.2 的 gas 账已被实测推翻,**这一版还带着它**
>
> 本文件停在 v0.8 发布线上,而 §3.2 的两个前提**当天晚些时候就被实测证否**:
>
> | | 本文件写的 | 实测 |
> |---|---|---|
> | 每名 gas | 50,000 | **120,588**(5 次真实注册,2.4× 贵) |
> | 主网 gas price | 10 gwei | **0.079 gwei**(取样 1025 个区块,**126× 便宜**) |
>
> **净效果是 §3.2 的结论反过来**:每个子名约 **$0.024**,3,000 个 $72 —— 不是几万美元。
> 于是「免费子域名的 L1 gas 账扛不住」不成立,§3「不能整体迁移」也随之只剩架构一条依据。
>
> **修正版在 `v0.9-ensv2` 分支**(连同方案 B、原生可读实测,以及一个按主网现价现算的成本探针)。
> 这里不并过来,是因为维护者定了 ENSv2 相关工作留在实验分支;
> —— 也因此**上面那个探针的命令名在这条分支上不存在**,横幅里就不写它了:
> `docs-commands.test.ts` 会检查文档里提到的每个 `pnpm` 脚本是否真在 `package.json` 里,
> 而初版横幅正是栽在这一格(见本次修正的由来)。
> **但把一份已知错误的成本分析原样发出去,比不发更糟** —— 所以在门口挂这块牌子。
>
> ⚠️ 读 §3 时请记住:**那一节的成本论证是错的。** 架构论证仍然成立,
> 但它只说明按层拆正当,不说明整体迁 L1 不可行。
