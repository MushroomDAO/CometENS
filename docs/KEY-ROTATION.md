...
> ⚠️ **新旧地址不能是同一个。** 脚本会在发交易前拦下这种情况。
> 原因不显然:守卫「替补必须已授权」单独看是正确的,但它**不蕴含**「替补 ≠ 被吊销的那个」——
> 传同一个地址时 `signers(new)` 恰恰是 true(因为它就是那个旧签名者),守卫放行,
> 吊销之后剩余签名者为 **0**,而每一步都报成功。轮换时两个地址并排显示在屏幕上,复制错一个很现实。
>
> 脚本另有一道事后断言:`removeSigner` 成功后直接读链确认**仍存在有效签名者**,
> 否则打印 CRITICAL 与恢复命令并 exit 1。前置检查是对输入做推理,这一条读的是链上事实。
