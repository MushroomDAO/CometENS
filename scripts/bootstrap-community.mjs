...
console.log('')
console.log('  B. .env.local (local tooling only — never commit it):')
console.log(`       VITE_ROOT_DOMAIN=${root}`)
console.log(`       VITE_L2_RECORDS_ADDRESS=${deployedAt}`)
console.log('')
console.log(`  C. On Ethereum ENS, set the resolver for ${root} to your L1 resolver.`)
console.log('     This script deliberately does NOT do it for you: it is a transaction on a name')
console.log('     you own, from an account this tool has no business holding.')
console.log('')
console.log('  D. Verify, in this order:')
console.log('       pnpm check:chain                 # chainId + contract + owner')
console.log('       pnpm preflight                   # full configuration check')
console.log(`       node scripts/resolve-testnet.ts alice.${root}   # end-to-end resolution`)
console.log('')
